...
function StoryCard({ result, sources }: { result: ScriptResult; sources: GenerateResponse["sources"] }) {
  ...
  const download = () => {
    const body = `# ${result.title}\n\n**Engine:** ${meta.name} · ${result.model}\n**Hook:** ${result.hook}\n**Estimated runtime:** ${Math.ceil(result.estimatedSeconds / 60)} minutes\n**Evidence status:** ${result.factCheck?.status || "legacy draft"} · ${result.factCheck?.coverage || 0}% citation coverage\n\n## Voice-over script\n\n${result.narration}\n\n## Scene and B-roll plan\n${result.visualBeats.map(v => `- ${v}`).join("\n")}\n\n## Music and sound design\n${result.soundDesign.map(v => `- ${v}`).join("\n")}\n\n## CTA\n${result.cta}\n\n## YouTube title options\n${[result.title, ...result.titleIdeas].map(v => `- ${v}`).join("\n")}\n\n## Editorial rationale\n${result.whyItWorks}\n\n## Fact-check warnings\n${result.factCheck?.warnings.length ? result.factCheck.warnings.map(v => `- ${v}`).join("\n") : "- No automatic warnings."}\n\n## Sources\n${sources.map((s, i) => `- [${i + 1}] [${s.title}](${s.url})`).join("\n")}`;
  ...
  return <article className={`story-card ${meta.color}`}>
    ...
    {result.error ? <div className="error-box"><strong>This engine paused</strong><p>{result.error}</p></div> : <>
      ...
      <div className="story-meta"><span>{result.wordCount} words</span><span>~{result.estimatedSeconds}s</span><span>{result.scores.retention}% retention craft</span><span>{result.factCheck?.status || "legacy"} · {result.factCheck?.coverage || 0}% cited</span></div>
